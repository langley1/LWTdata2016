...
#Run David's scores
#david_scores <- function(ALL_matrix_DavidScore) {
  # Step 1: Proportion of wins (P_ij)
 # P <- ALL_matrix_DavidScore / (ALL_matrix_DavidScore + t(ALL_matrix_DavidScore))
  #P[is.na(P)] <- 0  # set NaN (0/0 cases) to 0
  
  # Step 2: Calculate w, l, w2, l2
 # w  <- rowSums(P)            # sum of P_ij (direct wins)
  l  <- colSums(P)            # sum of P_ji (direct losses)
  #w2 <- P %*% w               # weighted wins (indirect)
  l2 <- t(P) %*% l            # weighted losses (indirect)
  
  # Step 3: David’s Score = w + w2 – l – l2
  DS <- w + w2 - l - l2
  return(DS)
}

#david_scores(ALL_matrix_DavidScore)

#Now trying it with EloRating 
install.packages("EloRating")
library(EloRating)

pre_ds_results<- DS(ALL_matrix_DavidScore)
...
*Calculating David's Scores*
```{r}
post_ds_results<- DS(ALL_matrix_post)
```
...
combined_grooming_matrix_post<- combined_grooming_matrix_post[,c(4,5,6,1,2,7:18,3)] #re-ordeing the categorical columns so they're in the front
```
