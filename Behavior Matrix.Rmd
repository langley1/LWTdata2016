...
monkeylist<-list(actor=MonkeyIDs_fems,recipient=MonkeyIDs_fems) #create list of all possible actors/recipients
...
grooming_matrix_rank <- grooming_matrix_tbl %>% #creating a new tibble with rank, sex, and age columns (Pops starts as 12 for highest ranked male)
  add_column(rank = c(3,2,5,16,1,7,6,13,8,15,10,9,17,18,12,11,4,14), .before = "AL") %>%
  arrange(rank) %>% #arranging by rank from 1-18
  add_column(sex = c("F","F","F","F","F","F","F","F","F","F","F","M","M","M","M","M","M","M"), .before ="AL") %>%
  add_column(age = c("A","A","A","A","A","A","A","J","J","J","A","A","A","A","SA","SA","SA","J"), .before = "AL")
grooming_matrix_rank

#Focal grooms
grooming_matrix2_rank<- grooming_matrix2_tbl %>%
  add_column(rank = c(3,2,5,16,1,7,6,13,8,15,10,9,17,18,12,11,4), .before = "AL") %>%
  arrange(rank) %>%
  add_column(sex = c("F","F","F","F","F","F","F","F","F","F","F","M","M","M","M","M","M"), .before ="AL") %>%
  add_column(age = c("A","A","A","A","A","A","A","J","J","J","A","A","A","SA","SA","SA","J"), .before = "AL")
grooming_matrix2_rank

#Females only
grooming_matrix_rank_fems<- grooming_matrix_rank %>% 
  filter(rank < 12) #filtering out ranks less than 12 to pull out only females
...
combined_grooming_matrix_post<- combined_grooming_matrix_post[,c(16,17,18,1:15)] #re-ording the categorical columns so they're in the front
```
