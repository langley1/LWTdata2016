...
post_focals_glmm<- post_focals %>%
  filter(AGE != "I", !FOCAL.ID %in% c("RE","PA","SK"), !POSITION.IN.CANOPY %in% c("MMS","NS1")) %>%
  droplevels() %>%
...
fm2 = clmm(PIC~ sex + age + (1|ID), data = post_focals_glmm, threshold = "equidistant")
summary(fm2)
```
...
monkeyID_surv_vig<- monkeyID_surv_vig %>%
  purrr::map2_dfc(~summarize(.,mean(count)))

out <- map2_df(.x = monkeyID_surv_vig,
               .y = names(monkeyID_surv_vig),
               .f = ~ {
                 mean <- mean(count) == .y]
                 tibble(data = .y, long.col = .x[[mean]])
                 })
map_chr(monkeyID_surv_vig[1:3], "names")
