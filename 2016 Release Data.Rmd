...
### Purrr method for SD
```{r}
combo_SD<- curl("https://raw.githubusercontent.com/langley1/LWTdata2016/main/Combo2.csv")
combo_SD<- read.csv(combo_SD, header = T)
combo_SD<- data.frame(lapply(combo_SD, as.character), stringsAsFactors=FALSE) %>%
  as_tibble() #Making this into a tibble with characters to see if it helps with the code below
head(combo_SD)
...
monkey.prelim_SD2 <-
  monkey.prelim_SD %>% 
  #purrr::map(~filter(.,MONTH=="2")) %>%
  purrr::map(~gather(.,"Obs", "behavior", 22:23, na.rm = T))
  #purrr::map(~group_by(.,behavior)) %>%
...
months<- c(1:12)
IDs<- unique(pre_focals$FOCAL.ID,na.rm=T)
IDs<- IDs[!IDs %in% c("OL","NY")]
IDs
month<- c(1:12)
combo_SD<- data.frame(IDs,Month)
str(combo_SD)
...
TEST<- combo_SD %>%
  mutate(SD = map2_dfr(
    IDs, 
    Month, 
    ~monkey.prelim_SD2 %>% pluck(.x) %>% filter(Month==.y) %>% 
    group_by(.,behavior) %>%
    summarize(.,count=n()) %>%
    as.data.frame(.)))
TEST
...
TEST<- combo_SD %>%
    ...
    IDs,
    Month, 
    lapply(monkey.prelim_SD2, function(x){.[.[["MONTH"]] == x, ]})
    ~monkey.prelim_SD2 %>% pluck(.x)) %>% filter(Month==.y) %>% 
    group_by(.,behavior) %>%
    summarize(.,count=n()) %>%
    as.data.frame(.)
...
Blah<- function(count) {
  answer<- group_by(behavior) %>%
  summarize(count=n())
  return(answer)
}

Attempt<- monkey.prelim_SD %>%
  lapply(monkey.prelim_SD2[30:31], Blah) 

monkey.prelim_SD2 <-
  monkey.prelim_SD %>% 
  lapply(months, function(x){.[.[["MONTH"]] == x, ]}) %>%
  purrr::map(~gather(.,"Obs", "behavior", 22:23, na.rm = T)) %>%
  purrr::map(~group_by(.,behavior)) %>%
  purrr::map(~summarize(.,count=n()))
...
monkey.prelim_SD2<- 
  monkey.prelim_SD2 %>%
  arrange(.monkey.prelim_SD2(desc(count)))
...
Pops_vig<- Pops_vig$count/nrow(Pops)
Pops_vig$ID<- "Pops"
Pops_vig
...
vig_plot<- barplot(vig_df_ALL$count, names.arg = vig_df_ALL$ID, cex.lab = 1, font.lab= 2, cex.names = 0.8, las=2, col="#339999", ylab = "Vigilance Counts", ylim= c(0,2000), density=c(100,100,100,100,100,100,100,100,100,100,20,100,20,100,20,20,20,20), legend = TRUE, args.legend = list(bty="n", x="topleft")) #Can't get the legend to work here!
vig_plot
...
MonkeysID<- c("PO", "ZI", "TI", "BL")
pre_cont_FULL$

for (val in MonkeysID) {
  val_G <- pre_cont_FULL %>%
    filter(focal.ID=="val") %>%
    filter(behav=="G+") %>%
    group_by(association) %>%
...
monkey_g<-lapply(pre_cont_FULL, function(x){pre_cont_FULL[pre_cont_FULL[["focal.ID"]] == x, ]})
```

```{r}
pre_focals %>% filter(FOCAL.ID == "PO") %>% map_df(~(data.frame(n_distinct = n_distinct(.x),
                                  class = class(.x))),
                     .id = "variable")
```
