...
monkey.prelim_SD3 <-
  ...
  purrr::map(~gather(.,"Obs", "behavior", 20:21, na.rm = T)) %>%
...
SD_combined<- SD_combined %>%
  mutate(IDs = fct_relevel(IDs, 
            "Pops","Jack", "Blue", "Alex", "Neville", "Augustine", "Amy", "May", "Toni", "Boo", "Big Mama", "Kovu", "Eddy", "Tinker", "Mango","Bart","Zip"))
...
wilcox.test(SD.Index ~ release, data = SD_combined2, paired = TRUE,
       alternative = "greater")
```
RESULTS: The troop's mean Shannon Diversity pre-release is significantly greater than the troop's mean Shannon Diversity post-release. 
...
```{r}
#Pre-release
pre_SDs_sex_plot<- boxplot(SD.Index ~ sex,
        data = pre_SDs_table,
        ylab="Shannon-Weaver Index (H)",
        xlab="Sex")
pre_SDs_sex_plot

pre_SDs_age_plot<- boxplot(SD.Index ~ age,
        data = pre_SDs_table,
        ylab="Shannon-Weaver Index (H)",
        xlab="Age")
pre_SDs_age_plot
...
#Post-release
post_SDs_sex_plot<- boxplot(SD.Index ~ sex,
        data = post_SDs_table,
        ylab="Shannon-Weaver Index (H)",
        xlab="Sex")
post_SDs_sex_plot

post_SDs_age_plot<- boxplot(SD.Index ~ age,
        data = post_SDs_table,
        ylab="Shannon-Weaver Index (H)",
        xlab="Age")
post_SDs_age_plot
...
```
RESULTS: There is a significant difference in mean Shannon Diversity values across sexes pre-release (p= 0.002) and post-release (p= 0.013). There is no significant difference in mean Shannon Diversity values across age groups pre-release (p= 0.311) or post-release (p= 0.426). There is no significant difference in mean Shannon Diversity values across social rank order pre-release (p=0.453) or post-release (p= 0.45).
